...
interface Portefolio {
    title: string;
    url: string;
}

export default function Progression(): JSX.Element {
    const [selectedPortfolio, setSelectedPortfolio] = useState<Portefolio | null>(null);
    ...
    const getAge = (date: Date) => new Date().getFullYear() - date.getFullYear();
    return (
        <div className={s.progressionPage}>
            ...
            {isModalOpen && selectedPortfolio && (
                ...
                >
                    <iframe src={selectedPortfolio.url} width="100%" height="100%" frameBorder="0"
                            className={s.PortfolioIframe}/>
                </Modal>
            )}
